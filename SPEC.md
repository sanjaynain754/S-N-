...
A project manifest is stored as `snp.toml`. The supported format has a `[package]` section with `name`, `version`, and optional `entry` (default `src/main.snp`), plus a `[dependencies]` section. Dependencies may be local paths or registry references. `snp init <directory> [name]` creates a starter project, and `snp package-check [snp.toml]` validates the manifest, resolves dependency manifests, checks the entry source, and validates import namespaces.

Imports beginning with `std.` are checked against the built-in standard-library registry. Other import roots must match a declared dependency, so a dependency named `util` can provide `util.math`. Registry dependencies use `name = { registry = "default", version = "^1.0.0" }`. With `SNP_REGISTRY` set, the filesystem registry layout is `REGISTRY/name/version/snp.toml`; the resolver selects the highest matching version and recursively resolves its registry dependencies. HTTP transport and package archives remain future work.
...
`snp lock snp.toml` resolves local and, when `SNP_REGISTRY` is set, registry dependencies and writes `snp.lock`. Each locked package records its exact version, source, and a deterministic checksum of its manifest. Registry resolution is recursive, selects the highest matching version, and rejects cycles, missing versions, malformed metadata, and incompatible constraints for one package name. `snp package-check snp.toml` validates locked versions and checksums. A lockfile is therefore a reproducibility and local-integrity mechanism; HTTP index access, archive downloads, content-addressed caching and cryptographic hashing remain future work.

## HTTP registry protocol v1

The remote registry protocol uses a versioned `/v1` namespace. `GET /v1/index/{package}.json` returns a JSON index document containing `protocol`, `package`, and a `versions` array. Each version entry contains the exact semantic version, canonical metadata URL, archive URL, dependency constraints, and an immutable `sha256:<hex>` archive checksum.

`GET /v1/packages/{package}/{version}/snp.toml` returns canonical package metadata. `GET /v1/packages/{package}/{version}/download` returns the package archive. The client must verify that the returned package identity and exact version match the request, validate the protocol marker `snp-registry-v1`, sort versions itself, choose the highest matching version, and verify the archive checksum before caching or extracting it.

The initial client contract uses `application/vnd.snp.registry.index+json;v=1` for indexes and `application/vnd.snp.package.metadata+toml;v=1` for metadata. Network failures and HTTP 5xx responses may be retried; malformed protocol data, HTTP 4xx responses, package mismatches and checksum failures are hard errors. Archives are treated as untrusted data: absolute paths, `..` traversal and build-script execution are prohibited. Full HTTP transport, archive extraction and persistent cache implementation follow this protocol design.
